...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const idem = (req.headers.get("Idempotency-Key") || req.headers.get("idempotency-key") || "").trim();
  ...
  const { data: form } = await sb
    .from("booking_forms").select("definition, status").eq("tenant_id", tenantId).eq("slug", slug).eq("status", "published").maybeSingle();
  ...
  if (!def.services.ids.includes(body.service_id)) return NextResponse.json({ error: "SERVICE_NOT_ALLOWED" }, { status: 400 });

  // Check for idempotent existing booking first
  const { data: existing } = await sb
    ...
    .maybeSingle();
  if (existing) {
    return NextResponse.json(existing, { status: 200, headers: { "Idempotency-Key": idem } });
  }

  // Load service and recompute using existing engine
  ...
  const { data: bookingRow, error: insertErr } = await sb
    ...
    .insert({
      ...
      snapshot: { form_slug: slug, service_id: body.service_id, frequency: body.frequency, inputs: {}, answers: body.answers, quote },
      customer: body.customer,
      idempotency_key: idem,
    })
    .select("id, status, currency, total_minor, vat_minor, rut_minor, created_at")
    .single();
  if (insertErr) return NextResponse.json({ error: "DB_ERROR", detail: insertErr.message }, { status: 500 });

  await sb.from("audit_logs").insert({ tenant_id: tenantId, action: "booking_created", entity: "booking", entity_id: bookingRow.id, meta: { form_slug: slug } });

  return NextResponse.json(bookingRow, { status: 201, headers: { "Idempotency-Key": idem } });
}
